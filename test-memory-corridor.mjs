...
import { CLUES } from './src/game/data/clues.ts';
...
const runEndingScenario = async (scenarioName, choices, expectedEnding) => {
  console.log(`\n\n========================================`);
  ...
  try {
    ...
    store.state.currentChapter = 'chapter_5';
    ...
    const phase2Clues = ['clue_cor_1', 'clue_cor_2'];
    ...
    const interactResult1 = store.interactWithMechanism('mech_cor_childhood_sort');
    logResult(interactResult1.success && interactResult1.type === 'memory_sort', `机关交互类型: ${interactResult1.type}`);

    const correctOrderPhase2 = ['clue_cor_1', 'clue_cor_2'];
    const sortResult1 = store.submitMemorySort('mech_cor_childhood_sort', correctOrderPhase2);
    ...
    const interactResult2 = store.interactWithMechanism('mech_cor_youth_choice');
    ...
    const choiceResult1 = store.submitBranchChoice('mech_cor_youth_choice', choice1);
    ...
    const phase4Clues = ['clue_cor_5', 'clue_cor_6', 'clue_cor_7', 'clue_cor_8'];
    ...
    const interactResult3 = store.interactWithMechanism('mech_cor_complete_puzzle');
    ...
    const sortResult2 = store.submitMemorySort('mech_cor_complete_puzzle', correctOrderPhase4);
    ...
    logStep(10, '阶段5：最终抉择');
    const interactResult4 = store.interactWithMechanism('mech_cor_final_choice');
    ...
    const choiceResult2 = store.submitBranchChoice('mech_cor_final_choice', choice2);
    logResult(choiceResult2.success, `选择: ${choice2}`);
    log('📝', `结果: ${choiceResult2.consequence}`);

    logStep(11, '解锁终章回廊');
    const result3 = store.solveMechanism('mech_cor_ending_door');
    ...
    logStep(13, '测试错误场景验证');
    const wrongSortResult = store.submitMemorySort('mech_cor_childhood_sort', ['clue_cor_2', 'clue_cor_1']);
...
const main = async () => {
  ...
  const scenarios = [
    {
      name: '🌟 真结局·永恒的守护者',
      choices: { choice1: 'choice_chase_dream', choice2: 'choice_eternal_treasure' },
      expectedEnding: 'ending_true'
    },
    {
      name: '💫 好结局·薪火相传',
      choices: { choice1: 'choice_chase_dream', choice2: 'choice_pass_torch' },
      expectedEnding: 'ending_good'
    },
    {
      name: '🌙 普通结局·时光的过客',
      choices: { choice1: 'choice_pursue_stable', choice2: 'choice_let_go' },
      expectedEnding: 'ending_neutral'
    },
    {
      name: '💔 坏结局·遗忘的代价',
      choices: { choice1: 'choice_escape', choice2: 'choice_forget' },
